...
export default function Changelog() {
  ...
  useEffect(() => {
    api.get('/api/changelogs/config').then(setConfig).catch(() => {});
    api.get('/api/changelogs/front')
    ...
    <div className="max-w-3xl mx-auto px-4 py-10">
      ...
      {config && (
        <p className="text-sm opacity-70 mb-8">
          Versión actual: <span className="font-bold text-accent-main">{config.current_version}</span>
        </p>
      )}
      {!versions.length && (
        ...
        {versions.map((v, idx) => (
          <div key={v.version} className="card overflow-hidden">
            ...
            >
              ...
              <span className="text-xs opacity-60 shrink-0">{v.date}</span>
              {v.author && <span className="text-xs opacity-50 italic shrink-0">por {v.author}</span>}
            ...
            {open === idx && (
              <div className="px-4 pb-4 border-t border-black/10 dark:border-white/10 pt-3 animate-fadeIn space-y-3">
                {v.sections?.map((sec, si) => (
                  <div key={si}>
                    <h3 className="font-bold text-sm mb-1 opacity-80">{sec.name}</h3>
                    <ul className="text-sm space-y-1 opacity-80 list-disc list-inside">
                      {sec.items.map((item, ii) => {
                        const text = item.replace(/^-\s*\*\*(.+?)\*\*\s*/, (_, b) => '').trim();
